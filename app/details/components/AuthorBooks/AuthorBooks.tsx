...
const AuthorBooks = () => {
  ...
  const { data, isFetching, isLoading } = useQuery({
    ...
    queryFn: () => getBooksByAuthorIds(page, String(authorId)),
  ...
  useEffect(() => {
    if (data) {
      const booksData = get(data, "data.data.data", []);
      const bookIds = books.map((book) => book._id);
      const uniqueId = new Set(bookIds);

      setBooks((prev) => {
        const filteredArr = booksData.filter(
          (book: IBook) => !uniqueId.has(book._id)
        );
        return prev.concat(filteredArr);
      });
